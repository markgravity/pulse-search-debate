...
class SearchBloc extends Bloc<SearchEvent, SearchState> {
	...
	Stream<SearchState> mapEventToState(
		...
		else if (event is SearchStartEvent && state is SearchWaitingForResponseState) {
		...
		else if (event is SearchSkipEvent && state is SearchWaitingForResponseState) {
			_waitingForResponseTimer?.cancel();
			matchingService.skip(state.matching.id)
				.then((_) {})
				.catchError((_) {});
	...
	void _scheduleCountingDownStatus() {
		...
		statusBloc.add(SearchStatusChangeContentEvent(content: "Begin in $seconds"));
	...
	void _scheduleChangeSearchingStatus() {
		...
		// Initial content
		statusBloc.add(SearchStatusChangeContentEvent(content: waitingStatus[_searchingStatusIndex]));

		_changeSearchingStatusTimer = Timer.periodic(_searchingStatusInterval, (timer) {
	...
	void _scheduleWaitingForResponseTimeoutEvent() {
		...
		statusBloc.add(SearchStatusChangeContentEvent(content: "Waiting for your response (${seconds}s)"));
		_waitingForResponseTimer?.cancel();
		_waitingForResponseTimer = Timer.periodic(Duration(seconds: 1), (timer) {
			...
			if (seconds <= 0) {
				timer.cancel();
				add(SearchSkipEvent(id: (state as SearchWaitingForResponseState).matching.id));
				return;
			}
	...
	void _listenSkipByOtherNotifications() {
		notiService.listen("matching_skip_by_other")
			.then((notification) {
